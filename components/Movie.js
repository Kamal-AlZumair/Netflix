...
const Movie = ({item}) => {
  ...
  const movieID = doc(db,'users', `${user_name?.email}`)
  ...
  return (
      <div  className='w-[160px] sm:w-[200px] md:w-[240px] lg:w-[280px] inline-block cursor-pointer relative p-2'>
        <Image width={'300'} height={'150'} className='w-full h-auto block'  alt={`${item?.title}`} src={`https://image.tmdb.org/t/p/w500/${item?.backdrop_path}`}/>
