...
const SavedShow = () => {
  ...
  useEffect(()=>{
    onSnapshot(doc(db, 'users',`${user_name?.email}`), (doc)=>{
      setMovies(doc.data()?.savedShows);
    })
  },[user_name?.email])

  const movieRef = doc(db, 'users', `${user_name?.email}`);
      ...
      <div className=' relative flex items-center group'>
        ...
        <div id={'slider'} className=' w-full h-full overflow-x-scroll whitespace-nowrap scroll-smooth scrollbar-hide relative'>
          {movies.map((item,id)=>(
            ...
            <Image width={'300'} height={'150'} className='w-full h-auto block'  alt={`${item?.title}`} src={`https://image.tmdb.org/t/p/w500/${item?.img}`}/>
